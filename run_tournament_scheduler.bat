...
echo Compiling...
"%JAVA_HOME%\bin\javac.exe" --module-path "%JAVAFX_HOME%\lib" --add-modules javafx.controls -cp "lib\*" -d out src/tournament/scheduler/*.java src/tournament/scheduler/masterdata/*.java
...
echo Running...
"%JAVA_HOME%\bin\java.exe" --module-path "%JAVAFX_HOME%\lib" --add-modules javafx.controls --add-opens javafx.graphics/com.sun.glass.utils=ALL-UNNAMED --add-opens javafx.graphics/com.sun.marlin=ALL-UNNAMED --enable-native-access=ALL-UNNAMED -cp "out;lib\*" tournament.scheduler.MainApplication
